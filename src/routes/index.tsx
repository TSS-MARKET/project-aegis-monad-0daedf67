...
function Landing() {
  ...
  return (
    <div className="min-h-screen relative overflow-x-clip">
      ...
      <section id="about" className="relative mx-auto max-w-[1560px] px-6 md:px-10 pb-24">
        <div
          style={{
            fontFamily: MONO,
            fontSize: "0.66rem",
        ...
        >
          A full <em style={{ color: "#22d3ee" }}>intelligence stack</em> for the Monad economy.
        </h2>
        <p className="mt-4 max-w-3xl" style={{ fontFamily: SANS, fontSize: "1.02rem", lineHeight: 1.65, color: "rgba(245,247,250,0.72)" }}>
          Aegis is a read only, non custodial dashboard that watches Monad in real time and translates raw on chain activity
          into evidence backed briefings, opportunities and wallet strategy. Nothing here is scraped from Twitter or generated
          from vibes. Every headline, score and recommendation is anchored to a specific event id you can click, inspect and
          replay.
        </p>
        ...
        <div className="mt-10 grid md:grid-cols-3 gap-4">
          {[
            {
              t: "How it works",
              b: "Aegis ingests block, mempool and DEX activity from Monad, ranks it through a deterministic scoring engine, then hands the top signals to an AI reasoning layer that must cite the underlying evidence before it speaks.",
            },
            {
              t: "What you get",
              b: "Live Market Radar across every major chain with Monad pinned first, an Opportunity Engine with catalysts and invalidation levels, Wallet DNA profiling, an Intelligence Timeline and Replay the Chain scrubber for post trade analysis.",
            },
            {
              t: "Built for daily use",
              b: "Ask Aegis is a copilot for your money. Send stablecoins, track a paycheck, watch a whale, price a coffee run, or get a morning brief before the market opens. Monad makes it cheap enough to matter every day.",
            },
            {
              t: "On chain and non custodial",
              b: "Connect a Monad wallet to attest AI briefs, manage watchlists and hold an Aegis subscription entirely on chain through the AegisRegistry contract. Aegis never takes custody of funds and never signs transactions on your behalf.",
            },
            {
              t: "Stack",
              b: "TanStack Start on Cloudflare, viem for Monad RPC, a Lovable AI Gateway reasoning layer, and a deterministic evidence engine that keeps the model honest. Sub second dashboards, streaming chat, mobile first.",
            },
            {
              t: "Roadmap",
              b: "Mainnet subscription tiers, real time whale alerts pushed to wallets, per protocol strategy agents, and a public API so other Monad apps can embed Aegis intelligence directly.",
            },
          ].map((c) => (
            <div
              key={c.t}
              className="hover-lift rounded-[6px] p-5"
              style={{
                background: "linear-gradient(180deg, rgba(10,18,28,0.55), rgba(4,10,16,0.55))",
                border: "1px solid rgba(34,211,238,0.14)",
              }}
            >
              <div style={{ fontFamily: MONO, fontSize: "0.62rem", letterSpacing: "0.16em", textTransform: "uppercase", color: "#22d3ee" }}>{c.t}</div>
              <p className="mt-2" style={{ fontFamily: SANS, fontSize: "0.9rem", lineHeight: 1.6, color: "rgba(245,247,250,0.78)" }}>{c.b}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 grid md:grid-cols-4 gap-3">
          {[
            { k: "Chain", v: "Monad" },
            { k: "Chain ID", v: "10143 / 143" },
            { k: "Custody", v: "None" },
            { k: "License", v: "Read only" },
          ].map((s) => (
            <div key={s.k} className="rounded-[6px] px-4 py-3" style={{ background: "rgba(34,211,238,0.04)", border: "1px solid rgba(34,211,238,0.14)" }}>
              <div style={{ fontFamily: MONO, fontSize: "0.58rem", letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(245,247,250,0.5)" }}>{s.k}</div>
              <div className="mt-1" style={{ fontFamily: SANS, fontSize: "0.95rem", fontWeight: 700, color: "#f5f7fa" }}>{s.v}</div>
            </div>
          ))}
        </div>
      </section>
